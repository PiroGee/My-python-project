...
score = 0
question_count = 0
time_left = 60  # 1 minute in seconds
...
def start_timer():
    ...
    timer_label.config(text=f"Time left: {mins}:{secs:02d}")
    if time_left > 0:
        time_left -= 1
        root.after(1000, start_timer)
    else:
        output_label.config(text=f"⏰ Time's up! Your score: {score}/{question_count}", fg="red")
        calc_button.config(state=DISABLED)
...
def show_quiz():
    ...
    global timer_label
    timer_label = Label(root, text="Time left: 1:00", font=('Verdana', 14), bg="#f0f8ff", fg="blue")
